...
static void SPI_WriteByte(char data)
{
    while(RESET == spi_i2s_flag_get(SPI4, SPI_FLAG_TBE));
    spi_i2s_data_transmit(SPI4, data);
    while(SET == spi_i2s_flag_get(SPI4, SPI_FLAG_TRANS));
}
...
{
    gpio_mode_set(GPIOE, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6);
    gpio_output_options_set(GPIOA, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_12);
}
